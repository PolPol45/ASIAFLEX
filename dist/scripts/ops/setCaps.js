...
async function setCaps(params) {
  ...
  console.log(`\n📊 Current caps:`);
  console.log(`   Max Daily Mint: ${hardhat_1.ethers.formatEther(currentMaxDailyMint)} AFX`);
  console.log(`   Max Daily Net Inflows: ${hardhat_1.ethers.formatEther(currentMaxDailyNetInflows)} AFX`);
  console.log(`   Supply Cap: ${hardhat_1.ethers.formatEther(currentSupplyCap)} AFX`);
  console.log(`   Current Total Supply: ${hardhat_1.ethers.formatEther(currentTotalSupply)} AFX`);
  // Validate new caps
  if (params.supplyCap) {
  ...
  if (params.dryRun) {
    ...
    if (params.maxDailyMint) {
      console.log(`   New Max Daily Mint: ${hardhat_1.ethers.formatEther(params.maxDailyMint)} AFX`);
    }
    if (params.maxDailyNetInflows) {
      console.log(`   New Max Daily Net Inflows: ${hardhat_1.ethers.formatEther(params.maxDailyNetInflows)} AFX`);
    }
    if (params.supplyCap) {
      console.log(`   New Supply Cap: ${hardhat_1.ethers.formatEther(params.supplyCap)} AFX`);
    }
    return;
  }
  // Execute cap updates
  ...
  try {
    if (params.maxDailyMint) {
      console.log(`📤 Setting max daily mint to ${hardhat_1.ethers.formatEther(params.maxDailyMint)} AFX...`);
    ...
    if (params.maxDailyNetInflows) {
      console.log(
        `📤 Setting max daily net inflows to ${hardhat_1.ethers.formatEther(params.maxDailyNetInflows)} AFX...`
      );
    ...
    if (params.supplyCap) {
      console.log(`📤 Setting supply cap to ${hardhat_1.ethers.formatEther(params.supplyCap)} AFX...`);
    ...
    console.log(`\n📊 New caps:`);
    console.log(`   Max Daily Mint: ${hardhat_1.ethers.formatEther(newMaxDailyMint)} AFX`);
    console.log(`   Max Daily Net Inflows: ${hardhat_1.ethers.formatEther(newMaxDailyNetInflows)} AFX`);
    console.log(`   Supply Cap: ${hardhat_1.ethers.formatEther(newSupplyCap)} AFX`);
    // Save operation
    const operation = {
      type: "set_caps",
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--max-daily-mint" && i + 1 < args.length) {
      params.maxDailyMint = hardhat_1.ethers.parseEther(args[i + 1]).toString();
    } else if (args[i] === "--max-daily-net-inflows" && i + 1 < args.length) {
      params.maxDailyNetInflows = hardhat_1.ethers.parseEther(args[i + 1]).toString();
    } else if (args[i] === "--supply-cap" && i + 1 < args.length) {
      params.supplyCap = hardhat_1.ethers.parseEther(args[i + 1]).toString();
    }
  }
